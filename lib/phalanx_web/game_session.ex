defmodule PhalanxWeb.GameSession do
  @moduledoc """
  ...
  use PhalanxWeb, :verified_routes
  ...
  def on_mount(:new, %{"id" => game_id} = _params, session, socket) do
    ...
    if session_game_id do
      ...
      if session_game_id == game_id do
        case Phalanx.Game.rejoin_game(game_id, player_name, player_token) do
          :ok ->
            socket =
              socket
        ...
        Phalanx.Game.quit_game(session_game_id, player_token)
  ...
  def rejoin_game_from_session(session, socket) do
    ...
    if session_game_id do
      ...
      case Phalanx.Game.rejoin_game(session_game_id, player_name, player_token) do
        :ok ->
          socket =
            socket
