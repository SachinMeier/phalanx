defmodule Phalanx.DynamicSupervisor do
  @moduledoc """
  ...
  def new_game() do
    id = Phalanx.Game.new_game_id()

    spec = %{
      id: id,
      start: {Phalanx.Game, :start_link, [id]},
      restart: :temporary
    }
  ...
  def new_game(state) do
    state = %{state | id: Phalanx.Game.new_game_id()}

    spec = %{
      id: state.id,
      start: {Phalanx.Game, :start_link, [state.id, state]},
      restart: :temporary
    }
  ...
  def list_games() do
    ...
    |> Enum.map(fn {_id_undef, pid, _type, modules}
                   when pid != :restarting and modules == [Phalanx.Game] ->
