...
```
   ...
   (GameMode.check_       [count == 0]           [game_mode has max_turns]
    win_condition/1)
         │                      │                      │
         │                      │                      │
         └──────────────────────┼──────────────────────┘
...
|-------|--------|----------|
| Game creation | Set `game_mode` from config (default: `:elimination_standard`) | `Game.new_game/1` |
| Game start | Initialize `mode_state` (e.g., starting unit counts for rout) | `handle_call({:join, ...})` when transitioning to `:playing` |
| Each turn | Check win condition after damage phase | `Engine.execute_orders/2` |
| Game end | Set `winner` and `status = :finished` | `check_win_condition/1` |

### Mode State Tracking

Different game modes track different state:

| Mode | mode_state Contents |
|------|---------------------|
| `:elimination_standard` | `%{}` (no extra state needed) |
| `:siege_basic` | `%{turns_survived: 0, defender_losses: 0}` |
| `:rout` | `%{starting_counts: %{team_a: N, team_b: M}}` |

### Frontend Exposure

Game mode selection is not exposed to the frontend initially. Backend defaults to `:elimination_standard`.
...
|-------|-----------|--------|
| `elimination` | `GameMode.check_win_condition/1` returns `{:winner, team}` | Winner set, status = :finished |
| `draw` | `GameMode.check_win_condition/1` returns `{:draw}` | winner = :draw, status = :finished |
| `continue` | `GameMode.check_win_condition/1` returns `{:continue}` | Proceed to next turn |
...
6. **Mode selection**: When should game mode be selectable? At game creation only, or changeable in lobby?
7. **Asymmetric modes**: How to handle different win conditions per team in the UI?
