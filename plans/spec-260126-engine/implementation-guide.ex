...
#
# Dependencies:
#   Hex -> Unit -> Group -> Force -> Combat modules -> Engine.Combat
...
defmodule Phalanx.Group do
  ...
  @doc """
  Detects all phalanx groups from current unit positions.
  Uses flood-fill on same-color, same-rotation adjacency.
...
defmodule Phalanx.Force do
  ...
  Force = Base(1) + Formation(0-4) + Support(0-2) + Flanking(0-2)
  """
  ...
  ## Example
      ...
      iex> Phalanx.Force.calculate_attack_strength({1, 1}, {2, 1}, units, [])
      1
  """
  ...
  @doc """
  Counts adjacent allies with same facing (side cohesion).
  Capped at max_side_bonus.
  """
  ...
  @doc """
  Counts rear allies with same facing (depth bonus).
  Capped at max_rear_bonus.
  """
  ...
  ## Example
      ...
      iex> Phalanx.Force.flanking_bonus(:east, 60)  # Attacking from east, defender faces NE
      1  # Flank attack
  """
...
defmodule Phalanx.Combat do
  ...
  def resolve_conflict(attacker_pos, defender_pos, units, supporters, _groups) do
    attack_strength = Force.calculate_attack_strength(attacker_pos, defender_pos, units, supporters)
    defense_strength = Force.calculate_defense_strength(defender_pos, units)
  ...
  def calculate_damage(attacker_pos, defender_pos, units) do
    ...
    base_damage = 1
    angle_bonus = Force.flanking_bonus(attack_dir, defender.rotation)
  ...
  @doc """
  Applies majority rule for phalanx groups.
  If majority of group would balk, all members balk.
  """
  @spec apply_majority_rule(map(), [Group.t()]) :: map()
  def apply_majority_rule(results, groups) do
    Enum.reduce(groups, results, fn group, acc ->
      group_results = group.positions
        |> MapSet.to_list()
        |> Enum.map(&Map.get(acc, &1, :hold))

      balk_count = Enum.count(group_results, &(&1 == :balk))
      total = length(group_results)

      if balk_count > total / 2 do
        # Majority balks: convert all moves to balks
...
defmodule Phalanx.Engine.Combat do
  @moduledoc """
  Combat engine implementing the 10-phase resolution algorithm.
  Orchestrates grouping, force calculation, and combat resolution.
  """

  @behaviour Phalanx.Engine

  alias Phalanx.{Hex, Unit, Group, Force, Combat, Order}
  ...
  def execute_orders(state, orders) do
    ...
    # Apply majority rule
    results = Combat.apply_majority_rule(results, snapshot.groups)
  ...
  defp resolve_single_conflict({:destination, _dest, contestants}, snapshot, support_data, results) do
    ...
    strengths = Enum.map(contestants, fn {from, to, _unit} ->
      ...
      strength = Force.calculate_attack_strength(from, to, snapshot.units, supporters)
      {from, strength}
    end)
