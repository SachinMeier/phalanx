...
The current engine implements simplified conflict detection. Full 10-phase resolution with:
...
Currently: None (games are ephemeral)

If persistence added:
- Write per turn: ~2 KB
- 1,000 games × 60 turns/hr = 120 MB/hr writes
- SQLite or ETS would suffice for single-node
