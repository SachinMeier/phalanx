...
| Category | Definition |
|----------|------------|
| **Side allies** | Adjacent phalanx members at 60° or 120° from facing (flanks) |
| **Rear allies** | Adjacent phalanx members at 180° from facing (directly behind) |
...
Maximum possible:
- Side allies: 2 (one on each side)
- Rear allies: 1 (directly behind)
- Total formation neighbors: 5 (front excluded as that's enemy direction)

**Note**: Only units in the SAME PHALANX count for formation bonuses. Adjacent allies not in the same phalanx provide no bonus.

---

## Test Formations

All units face the same direction (→ = facing right/East) and are in the same phalanx.
...
Each unit has 1 side ally in the same phalanx. All formulas give strength 2 (except F4: 1.75).
...
|------------------|----------------------------|------------------------|
| Front | All 5 formation neighbors | 4 |
| Flank (one side) | 3 neighbors (opposite flank + rear) | 3 |
| Flank (120°) | 2 neighbors (rear + partial opposite) | 2 |
| Rear (180°) | 0-1 neighbors | 1 |
...
| **F2** | Capped; simple | Global cap = arbitrary | Viable |
| **F3** | Separate caps make sense | 2+2 split is arbitrary | Balanced |
| **F4** | Diminishing returns = realistic | Fractional math; harder to grok | Complex |
| **F5** | Tiered = clear breakpoints | Step functions feel gamey | Viable |
...
Rationale:
- Side bonus naturally capped at +2 by geometry (only 2 side neighbors)
- Rear bonus uncapped—deep formations are powerful but vulnerable to flanking
- Self-balancing: players who stack too deep get flanked and destroyed
- Mental model: "Width protects your flanks, depth adds pushing power"
...
def effective_strength(unit, attack_direction) do
  ...
  sides = count_phalanx_side_allies(relevant_neighbors)
  rear = count_phalanx_rear_allies(relevant_neighbors)
...
### Explicit Phalanx Membership Requirement

**Formation bonuses apply only to explicit phalanx members.** Phalanxes are declared during battle, not auto-detected based on adjacency or facing.

Units only count for formation bonuses if they:
1. Are in the **same phalanx** as the unit being evaluated
2. Have the **same facing direction**
3. Are in a **side or rear position** relative to the unit

```elixir
def phalanx_neighbors(unit, units, phalanx_membership) do
  unit_phalanx = Map.get(phalanx_membership, unit.id)

  unit
  |> neighbors()
  |> Enum.filter(fn pos ->
    ally = Map.get(units, pos)
    ally_phalanx = Map.get(phalanx_membership, ally && ally.id)

    ally &&
      ally.color == unit.color &&
      ally.rotation == unit.rotation &&
      ally_phalanx == unit_phalanx &&
      unit_phalanx != nil
  end)
end
```

**Key changes from auto-detection model:**
- No "same movement direction" check (phalanxes move atomically)
- No auto-detection of formations based on proximity
...
**Critical rule:** Formation bonuses require **explicit phalanx membership**. Adjacent allies not in the same declared phalanx provide no bonus, regardless of facing or proximity.
